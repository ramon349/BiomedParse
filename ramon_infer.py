...
def main(): 
    transforms = Compose(
        [
        LoadImaged(keys=['image','label'],reader=NibabelReader,image_only=False),
        EnsureChannelFirstd(keys=['image','label']),
        Orientationd(keys=['image','label'],axcodes='RAS'),
        BiomedScaled(keys=['image'],site='abdomen'),
    ...
    with open(pkl_path,'rb') as f:
        ts =  pkl.load(f)[-1]
    tr_ds = Dataset([ts[-1]],transform=transforms)
    dl = DataLoader(tr_ds,batch_size=1,num_workers=4,persistent_workers=True)
    all_paths = list() 
    preds = list() 
    for data_sample in dl: 
        file_exists = check_if_exists(data_sample['label'],out_dir)
        if file_exists:
            continue
        slide_preds = sliding_window_inference(data_sample['image'][0].to(torch.float),roi_size=(1024,1024,1),sw_batch_size=1,predictor=lambda x: pred_wrap(model,x),sw_device='cpu',device='cpu',buffer_dim=-1,progress=True)
        data_sample['preds']= monai.data.MetaTensor(slide_preds[0,0,:,:,:],affine=data_sample['label'].meta['affine'],meta=data_sample['label'].meta)
